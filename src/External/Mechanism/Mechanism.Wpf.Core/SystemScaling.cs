...
namespace Mechanism.Wpf.Core
{
    ...
    public static class SystemScaling
    {
        ...
        {
            get => NativeMethods.GetCursorPos(out NativeMethods.POINT p) ? new Point(RealPixelsToWpfUnits(p.X), RealPixelsToWpfUnits(p.Y)) : new Point(0, 0);
            set => NativeMethods.SetCursorPos((int)RealPixelsToWpfUnits(value.X), (int)RealPixelsToWpfUnits(value.Y));
            //set =>System.Windows.Forms.Cursor.Position = new System.Drawing.Point((int)WpfUnitsToRealPixels(value.X), (int)WpfUnitsToRealPixels(value.Y));
        }
