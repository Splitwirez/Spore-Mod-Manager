...
using System.Text;
using static SporeMods.Context.GameInfo;
...
namespace SporeMods.Injection
{
	public static class SporeLauncher
	{
			...
			try
			{
				...
				if (File.Exists(_executablePath))
				{
					...
					if (IsValidExe())
					{
						...
						{
							MessageDisplay.RaiseError(new ModErrorEventArgs(ex)); //ex.Message + "\n\n" + ex.StackTrace
							return;
						}
		...
		static void InjectSteamSporeProcess()
		{
			...
			{
				IntPtr pOpenThread = InjNativeMethods.OpenThread(ThreadAccess.SUSPEND_RESUME, false, (uint)pT.Id);
				if (pOpenThread != IntPtr.Zero) //((pOpenThread != IntPtr.Zero) && (Injector.WaitForSingleObject(pOpenThread, 0) != WAIT_ABANDONED))
				{
					if (InjNativeMethods.SuspendThread(pOpenThread) == -1) //     r/therewasanattempt
					{
			...
			*/
			...
			foreach (IntPtr pOpenThread in pOpenThreads)
			{
				...
				{
					suspendCount = */
				if (InjNativeMethods.ResumeThread(pOpenThread) == -1)
				{
					ThrowWin32Exception("Thread resume failed (NOT LOCALIZED)");
				}
				//} while (suspendCount > 0);

				InjNativeMethods.CloseHandle(pOpenThread);
			...
			//}
			//}
			//ModInstallation.InvokeErrorOccurred(new ModErrorEventArgs(new Exception("EXPERIMENTAL INJECTION FAILURE")));
			//}

			//};   
		}

		static InjNativeMethods.MonitorInfoEx _monitor = new InjNativeMethods.MonitorInfoEx();
		...
		static string GetGameCommandLineOptions()
		{
			...
			else
			{
				if (Settings.ForceGameWindowingMode)
				{
					...
					var monitors = InjNativeMethods.AllMonitors;
					_monitor = InjNativeMethods.AllMonitors[0];
		...
		static void CreateSporeProcess()
		{
			...
			if (!InjNativeMethods.CreateProcess(null, "\"" + _executablePath + "\" " + sb.ToString(), IntPtr.Zero, IntPtr.Zero, false, ProcessCreationFlags.CREATE_SUSPENDED, IntPtr.Zero, GameInfo.SporebinEP1, ref _startupInfo, out _processInfo))
			{
		...
		{
			if (InjNativeMethods.ResumeThread(_processInfo.hThread) != 1)
			{
		...
		static void EnableBorderless(int pid, InjNativeMethods.MonitorInfoEx monitor)
		{
			if (Settings.ForceWindowedMode == 2)
			{
				...
				while (!InjNativeMethods.IsWindow(spore)) //(spore == IntPtr.Zero) ||  //(!process.HasExited) && ((process.MainWindowHandle == IntPtr.Zero) || (!NativeMethods.IsWindow(process.MainWindowHandle))))
				{
				...
				if (InjNativeMethods.IsWindow(spore)) //(!process.HasExited) && (process.MainWindowHandle != IntPtr.Zero) && NativeMethods.IsWindow(process.MainWindowHandle))
				{
					...
					int winStyle = (Int32)InjNativeMethods.GetWindowLong(spore, InjNativeMethods.GwlStyle);
					winStyle &= ~InjNativeMethods.WsBorder;
					winStyle &= ~InjNativeMethods.WsCaption;
					InjNativeMethods.SetWindowLong(spore, InjNativeMethods.GwlStyle, winStyle);
					...
					InjNativeMethods.SetWindowPos(spore, IntPtr.Zero, x, y, cx, cy, uFlags);
				}
			}
		}
		...
		*/
		...
		public static void ThrowWin32Exception(string info, int error)
		{
			if ((error != 0) && (error != 18))
			{
				...
				MessageDisplay.RaiseError(new ModErrorEventArgs(exception));
				throw exception;
			}
		}
		...
		public static bool IsValidExe()
		{
			...
			else if (TryGetExeVersion(_executablePath, out Version exeVersion))
			{
				...
				{
					MessageDisplay.RaiseError(new ModErrorEventArgs(new InvalidOperationException(GetLocalizedString("LauncherError!GameVersion!TooOld"))));
					return false;
				}
				else if (exeVersion > Spore__March2017)
				{
					MessageDisplay.RaiseError(new ModErrorEventArgs(new InvalidOperationException(GetLocalizedString("LauncherError!GameVersion!WaitDidTheyActuallyUpdateSpore"))));
					return false;
				}
				else
					return true;
			...
			{
				MessageDisplay.RaiseError(new ModErrorEventArgs(new InvalidOperationException(GetLocalizedString("LauncherError!GameVersion!ReadFailed"))));
				return false;
			}
		}
		...
		const string ORIGIN_PREREQ_1 = "SporeMods.Core.ModAPIFix.SporeApp_ModAPIFix.exe";
		const string ORIGIN_PREREQ_2 = "SporeMods.Core.ModAPIFix.steam_api.dll";

		/*public static bool HasOriginPrerequisites() =>
			HasOriginPrerequisites(out Stream _, out Stream _);*/

		public static bool HasOriginPrerequisites(out Stream prereq1, out Stream prereq2, bool close = true)
		{
			prereq1 = Assembly.GetExecutingAssembly().GetManifestResourceStream(ORIGIN_PREREQ_1);
			prereq2 = Assembly.GetExecutingAssembly().GetManifestResourceStream(ORIGIN_PREREQ_2);

			bool has1 = prereq1 != null;
			bool has2 = prereq2 != null;
			if (close)
			{
				if (has1)
					prereq1.Close();
		...
		{
			string resNames = string.Empty;
			foreach (string s in Assembly.GetExecutingAssembly().GetManifestResourceNames())
            {
				resNames += s + "\n";
			}
			MessageDisplay.ShowMessageBox(resNames);


			if (HasOriginPrerequisites(out Stream prereq1, out Stream prereq2, false))
			{
				using (prereq1)
				{
