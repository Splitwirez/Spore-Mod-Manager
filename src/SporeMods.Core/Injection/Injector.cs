...
namespace SporeMods.Injection
{
	class Injector
	{
		...
		public static void InjectDLL(PROCESS_INFORMATION pi, string dllPath)
		{
			...
			IntPtr retLib = InjNativeMethods.GetProcAddress(InjNativeMethods.GetModuleHandle("kernel32.dll"), "LoadLibraryW");
			...
			IntPtr hProc = InjNativeMethods.OpenProcess(InjNativeMethods.AccessRequired, false, pi.dwProcessId); //Open the process with all access
			if (hProc != IntPtr.Zero)
			{// Allocate memory to hold the path to the DLL file in the process' memory
				IntPtr objPtr = InjNativeMethods.VirtualAllocEx(hProc, IntPtr.Zero, (uint)dllPath.Length + 1, AllocationType.Commit, MemoryProtection.ReadWrite);
				...
				bool writeProcessMemoryOutput = InjNativeMethods.WriteProcessMemory(hProc, objPtr, bytes, (uint)bytes.Length, out numBytesWritten);
				...
				IntPtr lpThreadId;
				IntPtr hRemoteThread = InjNativeMethods.CreateRemoteThread(hProc, IntPtr.Zero, 0, retLib, objPtr, 0, out lpThreadId);

				// Wait for the thread to finish
				if (hRemoteThread != IntPtr.Zero)
				{
					if (InjNativeMethods.WaitForSingleObject(hRemoteThread, MAXWAIT) == WAIT_TIMEOUT)
					{
				...
				InjNativeMethods.VirtualFreeEx(hProc, objPtr, (uint)bytes.Length, AllocationType.Release);

				InjNativeMethods.CloseHandle(hProc);
			}
			else
			{
