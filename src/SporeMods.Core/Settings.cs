...
namespace SporeMods
{
	public static class Settings
	{
		...
		static string _buildType = null;
		public static string BuildType
		{
			/*
			#if (!DEBUG) && (!EXPLICIT_BUILD_TYPE)
				get => string.Empty;
			#else
			*/
			get
			{
				if (_buildType == null)
				{
#if EXPLICIT_BUILD_TYPE
using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("BUILD_TYPE"))
					{
						using (StreamReader reader = new StreamReader(stream))
						{
							_buildType = reader.ReadToEnd();
						}
					}
		...
		public static Version CurrentDllsBuild
		{
			get
			{
				...
				if (versions.Count() > 0)
				{
					...
					return ModIdentity.UNKNOWN_MOD_VERSION;
			}
		}
