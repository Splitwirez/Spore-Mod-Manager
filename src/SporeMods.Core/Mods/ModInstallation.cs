...
namespace SporeMods.Mods
{
	public static class ModInstallation
	{
		...
		public static async Task RegisterSporemodModWithInstallerAsync(string modName)
		{
			...
			{
				MessageDisplay.RaiseError(new ModErrorEventArgs(new Exception("This mod does not have a configurator!")));
			}
		}
	...
	public class ModErrorEventArgs : EventArgs
	{
		...
		public ModErrorEventArgs(Exception ex)
		{
			Title = Path.GetFileNameWithoutExtension(Process.GetCurrentProcess().MainModule.FileName);
			//Content = ex.Message + "\n" + ex.StackTrace;
			Exception = ex;
		...
		public ModErrorEventArgs(string title, Exception ex)
		{
			Title = title;
			//Content = ex.Message + "\n" + ex.StackTrace;
			Exception = ex;
