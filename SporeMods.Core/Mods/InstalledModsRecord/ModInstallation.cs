...
namespace SporeMods.Core
{
	public static class ModInstallation
	{
		...
		/*public static void ClearQueues()
		{
			...
			foreach (string d in Directory.EnumerateDirectories(Settings.ModsPath))
			{
		...
		public static string AnyInstallActivitiesPath = Path.Combine(SmmInfo.TempFolderPath, "InstallingSomething");
		...
		static bool IsNetworkPath(string path) =>
			SmmInfo.IsWindowsLike
				? PathIsNetworkPath(path)
				: false; //TODO: Proper non-Windows implementation
		public static async Task<ModInstallationStatus> InstallModsAsync(string[] modPaths)
		{
			IS_INSTALLING_MODS = true;
			//ModsManager.Instance.AddToTaskCount(modPaths.Count(x => !IsNetworkPath(x)));
		...
		public static async Task<Exception> RegisterLoosePackageModAsync(string path)
		{
			...
			{
				ManagedMod mod = null;
				if (Settings.Instance.AllowVanillaIncompatibleMods)
				{
					...
					string dir = Path.Combine(SmmInfo.ModsPath, noExtensionName);
		...
		public static async Task<Exception> RegisterSporemodModAsync(string path)
		{
			...
			try
			{
				...
				using (ZipArchive zip = ZipFile.OpenRead(path)) //ZipFile zip = new ZipFile(path))
				{
					...
					string dir = Path.Combine(SmmInfo.ModsPath, name);
					...
					Task validateModTask = new Task(() =>
					{
						if (zip.TryGetEntry(MOD_INFO, out ZipArchiveEntry entry))
						{
							...
							if (identityVersion > ModIdentity.XmlModIdentityVersion1_0_0_0)
							{
								...
								if (dllsBuildAttr != null)
								{
									...
									{
										if (dllsBuild > SmmInfo.CurrentDllsBuild)
							...
							if (uniqueAttr != null)
							{
								...
								dir = Path.Combine(SmmInfo.ModsPath, unique);
								name = unique;
							}
						...
						else if (Settings.Instance.AllowVanillaIncompatibleMods)
							CreateModInfoXml(name, SmmInfo.TempFolderPath, out document);

						if (proceed || Settings.Instance.AllowVanillaIncompatibleMods)
						{
							string[] modConfigDirs = Directory.EnumerateDirectories(SmmInfo.ModsPath).ToArray();
					...
					if (proceed || Settings.Instance.AllowVanillaIncompatibleMods)
					{
						try
						{
							...
							Task ensureOldFilesTask = new Task(() =>
							{
								...
								if (!isUnique)
								{
									if ((prevConfigDirPath != null) && Directory.Exists(prevConfigDirPath))
										Directory.Move(prevConfigDirPath, Path.Combine(SmmInfo.TempFolderPath, Path.GetFileName(prevConfigDirPath)));
							...
							if (!isUnique)
							{
								...
								{
									Directory.Delete(Path.Combine(SmmInfo.TempFolderPath, Path.GetFileName(prevConfigDirPath)), true);
								});
						...
						catch (Exception x)
						{
							...
							if (Directory.Exists(prevConfigDirPath))
								Directory.Move(Path.Combine(SmmInfo.TempFolderPath, Path.GetFileName(prevConfigDirPath)), prevConfigDirPath);
				...
				if (isUnique && (proceed || Settings.Instance.AllowVanillaIncompatibleMods))
		...
		public static async Task RegisterSporemodModWithInstallerAsync(string modName)
		{
			...
			{
				MessageDisplay.Error(new Exception("This mod does not have a configurator!"));
			}
		}
	}
