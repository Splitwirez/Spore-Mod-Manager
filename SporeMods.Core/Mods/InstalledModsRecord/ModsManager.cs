...
namespace SporeMods.Core
{
	public class ModsManager : NotifyPropertyChangedBase
	{
		...
		private void PopulateModConfigurations()
		{
			...
			foreach (string s in Directory.EnumerateDirectories(SmmInfo.ModsPath))
			{
		...
		public async Task<bool> ShowModConfigurator(ManagedMod mod)
		{
			...
			if (mod.HasConfigurator)
				return await MessageDisplay.ShowModal<bool>(new ModConfiguratorModal(mod)); //ModConfiguratorShown(mod);
			else
				return false;
		}
		...
		public static ModsManager Instance { get; set; } = new ModsManager();
		private ModsManager()
		{
	...
	public class ModConfiguratorModal : IModalViewModel<bool>
	{
		...
		public void ConfirmCommand(object parameter = null)
		{
			_tcs.TrySetResult(true);
		}


		ManagedMod _mod = null;
		public ManagedMod Mod
		{
			get => _mod;
		}
		public ModConfiguratorModal(ManagedMod mod)
		{
			_mod = mod;
		}
	}
