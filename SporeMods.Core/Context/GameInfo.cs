...
namespace SporeMods.Core
{
	public static class GameInfo
	{
		...
		static string EnsureGameInstallPathIsInstallRoot(string subPath, GameDlc dlc)
		{
			...
			else
			{
				...
				{
					Task<string> task = MessageDisplay.ShowModal<string>(new BadPathEventArgs(subPath, output, dlc));
					task.Start();
					task.Wait();
					return task.Result;
				}
		...
		public static string SporebinEP1
		{
			get
			{
				...
				if (!Settings.Instance.ForcedGalacticAdventuresSporebinEP1Path.IsNullOrEmptyOrWhiteSpace())
					return Settings.Instance.ForcedGalacticAdventuresSporebinEP1Path;
				else
		...
		public static string GalacticAdventuresData
		{
			get
			{
				if (!Settings.Instance.ForcedGalacticAdventuresDataPath.IsNullOrEmptyOrWhiteSpace())
					return Settings.Instance.ForcedGalacticAdventuresDataPath;
				else
		...
		public static string CoreSporeData
		{
			get
			{
				...
				if (!Settings.Instance.ForcedCoreSporeDataPath.IsNullOrEmptyOrWhiteSpace())
					return Settings.Instance.ForcedCoreSporeDataPath;
				else
		...
		{
			if (Settings.Instance.IgnoreSteamInstallInfo)
				return false;
			else
			{
		...
		public static List<AppPathGuess> GetFailureGuessFolders(GameDlc dlc, bool isSporebin)
		{
			List<AppPathGuess> guesses = new List<AppPathGuess>();
			...
			if (Directory.Exists(diskBase))
				guesses.Add(new AppPathGuess(diskBase, GameExecutableType.Disk__1_5_1));

			if (Directory.Exists(originBase))
				guesses.Add(new AppPathGuess(originBase, GameExecutableType.Origin__March2017));

			if (Directory.Exists(steamBase))
				guesses.Add(new AppPathGuess(steamBase, GameExecutableType.GogOrSteam__March2017));

			if (false && Directory.Exists(gogBase))
				guesses.Add(new AppPathGuess(gogBase, GameExecutableType.GogOrSteam__March2017));
	...
	public class BadPathEventArgs : EventArgs, IModalViewModel<string>
	{
		TaskCompletionSource<string> _tcs = new TaskCompletionSource<string>();
		public TaskCompletionSource<string> GetCompletionSource() => _tcs;
