...
namespace SporeMods.Core
{
    ...
    {
        class TempBackupFile : BackupFile
        {
            ...
            public TempBackupFile(string originalPath, bool safeWrite)
            {
        ...
        class MissingBackupFile : BackupFile
        {
            private readonly string originalPath;

            public MissingBackupFile(string originalPath, bool safeWrite)
            {
                this.originalPath = originalPath;
                _isValid = true;
                _safeWrite = safeWrite;
        ...
        class DirectoryBackupFile : BackupFile
        {
            private readonly string originalPath;
            private readonly string tmpBackupPath;
            private readonly string drive;
            ...
            public DirectoryBackupFile(string originalPath, bool safeWrite)
            {
                this.originalPath = originalPath;
        ...
        private static ConcurrentDictionary<BackupFile, int> backups = new ConcurrentDictionary<BackupFile, int>();
        ...
        /// <returns></returns>
        public static BackupFile BackupFile(string path, bool safe = true)
        {
            ...
            if (File.Exists(path))
            {
                backup = new TempBackupFile(path, safe);
            }
            else if (Directory.Exists(path))
            {
                backup = new DirectoryBackupFile(path, safe);
            }
            else
            {
                backup = new MissingBackupFile(path, safe);
            }
            backups[backup] = 0;
            return backup;
        }

        public static void DisposeBackup(BackupFile backup)
        {
            backup.Dispose();
