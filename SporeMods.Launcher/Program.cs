...
namespace SporeMods.Launcher
{
	class Program
	{
		...
		static void Main(string[] programArgs)
		{
			...
			{
				if (programArgs[i] == Cmd.RunUnderWineFromLinux)
				{
					wineRun = true;
					sbep1 = programArgs[i + 1];
					break;
				}*/
			//if (programArgs.Any(x => x.Trim('"').Equals("--wine", StringComparison.OrdinalIgnoreCase)))
				//SporeLauncher.LaunchGame(programArgs[1].Trim('"'), "Z:" + SmmInfo.ModLibsPath.Replace('/', '\\'), "Z:" + SmmInfo.CoreLibsPath.Replace('/', '\\'), "Z:" + SmmInfo.LegacyLibsPath.Replace('/', '\\'));
			

			/*#if LINUX_BUILD
			string sbep1 = Environment.GetCommandLineArgs().Last().Trim('"');
			SporeLauncher.LaunchGame(sbep1, Environment.ExpandEnvironmentVariables("%programdata%"), false);
			#else
			Console.WriteLine("RUN WITHOUT --wine");*/
